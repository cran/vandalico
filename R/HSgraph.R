...
HSgraph <- function(mat, breaks = 10, hist.total = TRUE) {
    ...
    plot(histpresence, col = rgb(0, 0, 1, 1/4), xlim = c(min(mat[, 1]), max(mat[, 1])), ylim = c(0, max(histpresence$counts,
        histabsence$counts, histtotal$counts)), border = rgb(0, 0, 1, 0), main = "", xlab = "Suitability", ylab = "Frequency",
        las = 1)
    plot(histabsence, col = rgb(1, 0, 0, 1/4), xlim = c(min(mat[, 1]), max(mat[, 1])), border = rgb(1, 0, 0, 0),
        add = T)
    if (hist.total == TRUE) {
        lines(c(0, histtotal$breaks), c(0, histtotal$counts, 0), type = "s", lwd = 2)
    }
}
